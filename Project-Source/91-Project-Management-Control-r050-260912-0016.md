...
supersedes: "91-Project-Management-Control-r049-260911-1549.md"
...
- **Success Criteria:** (1) TASK-047 durably registered; (2) current SKILL requires Markdown-safe three-field footer in order; (3) Scenario 432 exists and treats hidden labels as failure; (4) live ChatGPT UI visibility of all three safe fields is USER_CONFIRMED; (5) no unnecessary Framework semantic/version change occurs; (6) bounded verification passes; (7) local completion commit is observed; (8) Task/Goal terminal reconciliation is persisted.
- **Current Evidence:** `EVD-090`.
- **Related AUTH / ACT / ENV:** `AUTH-015 ACTIVE` / `ACT-027 IN_PROGRESS` / `ENV-015 ACTIVE`.
...
- **Remaining Work:** bounded GREEN verification → local completion commit → resulting-state readback → terminal reconciliation.
