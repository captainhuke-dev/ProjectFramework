...
supersedes: "00-Project-Source-Framework-r006-260916-1332.md"
framework_root: true
inherits_from: []
...
## 1. Framework 1.19 Pin and Incorporated Normative Source

This canonical ProjectFramework self-host is reconciled to **ProjectFramework 1.19.0 / Project Source Schema 1.0.0** under `MIG-005` as the Phase 0 prerequisite of TASK-058, authorized by ACTOR-001 on 2026-09-16. The reconciliation is `COMPLETED / PERSISTED / NOT_PENDING` on canonical `main`; this is canonical-upstream post-release self-host reconciliation to the already-verified Framework 1.19.0 distribution, not an ordinary consuming-Project `[Project Upgrade]`.

The incorporated Framework 1.19 semantics are bound to the exact verified release below. The Framework 1.19.0 distribution (TASK-057 AI-ControlTower Governance Support Layer) was released at candidate `f378e0a0b4796a94ec52ba1286d17d05e6a5c9b2` and preserves the independently verified Framework-Source tree `23274ada739c56a10c8edcfc14e6a9a0e46e9a0b`; TASK-057 is the release lineage layered on TASK-051/TASK-052 and the TASK-049 canonical self-hosting contract. These source files do **not** become a second Project authority: their pinned text is incorporated by this `FRAMEWORK-001`; this root remains Project governance authority.

```yaml
framework_source_provenance:
  repository: "captainhuke-dev/ProjectFramework"
  canonical_branch: "main"
  canonical_release_candidate_commit: "f378e0a0b4796a94ec52ba1286d17d05e6a5c9b2"
  self_host_reconciliation_source_ref: "task058-phase0-self-host-reconcile-1.19"
  reconciliation_task: "TASK-058"
  incorporated_framework_source_tree: "23274ada739c56a10c8edcfc14e6a9a0e46e9a0b"
  ...
  canonical_integration_commit: "5b08c5c1438aa9898daa13fae4dc00e0bcfc0df5"
```

Pinned Framework 1.19 source set incorporated here:

- `Framework-Source/FRAMEWORK-RELEASE.yaml` — blob `b58ca43a2c146483ae328582d4a48af1a067570e`
- `Framework-Source/SKILL.md` — blob `e14162fc9b1dc958efee2f83c208897cb1edd7cc`
...
- `Framework-Source/references/core-governance-rules.md` — blob `1cd62bf5e01b4aa90bdfae56919893ee511f4b81`
- `Framework-Source/templates/00-project-source-framework.md` — blob `0ab5b1ffba0f6c64e856f60e5f867389854579e8`
- `Framework-Source/templates/core-document-skeletons.md` — blob `d9bbdd664a02161b4cda2979e99f07c7e47afaf5`
...
`MIG-002` remains the historical `ASSESSED_PATH` 1.7.0 → 1.15.0 Project upgrade described in active `16`. `MIG-005` is the current canonical self-host post-release reconciliation to Framework 1.19.0 and is not an ordinary consuming-Project `[Project Upgrade]`. Preservation, rollback, evidence, history, verification, and promotion gates are not skipped.
...
## 10. Registered Commands and Current Framework 1.19 Response Semantics
...
Framework 1.19 mandatory visible response close in the pinned Framework semantics is:
...
Nothing follows `[Reason]` under the pinned Framework 1.19 contract. `[Next Goal]` is presentation-only and never creates authority. `[Chat]` and `[Required Read]` remain valid internal Handoff/continuation semantics but are not mandatory visible Framework 1.19 close fields.

## 11. Optional Framework 1.10–1.14 Surfaces

The cumulative 1.19 pin supports, without automatically materializing:
...
No `PSR-*` is materialized at this revision. Add a Project-Specific Rule only through a governed future `FRAMEWORK-001` revision with required approval; it may specialize but never weaken pinned Framework 1.19 semantics.
...
Project Source Framework Pin: 1.19.0
...
Self-Host Reconciliation: MIG-005 / CANONICAL_SELF_HOST_POST_RELEASE_RECONCILIATION / COMPLETED / PERSISTED / NOT_PENDING
```
