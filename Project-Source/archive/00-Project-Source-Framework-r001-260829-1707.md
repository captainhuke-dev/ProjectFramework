...
Framework Distribution Root (current 1.7.0): managing-project-source/
Google Drive: NOT_APPLICABLE
External File Storage: NOT_APPLICABLE
```

The current Framework distribution directory is distinct from this Project's authoritative `Project-Source/`. TASK-038 proposes a future Framework `1.8.0` rename of the distribution root to `Framework-Source/`; that roadmap item does not alter current `1.7.0` Project Source authority until separately implemented and verified.
