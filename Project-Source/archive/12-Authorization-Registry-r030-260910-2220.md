...
supersedes: "12-Authorization-Registry-r029-260910-2057.md"
...
## AUTH-012 — TASK-045 persistent Goal execution authority
- **Authority Type:** USER_EXPLICIT_PERSISTENT_GOAL_AUTHORIZATION
- **Granted By:** ACTOR-001
- **Granted At:** 2026-09-06T13:45:00+07:00
- **Purpose / Outcome:** complete TASK-045 Response Close + Next Goal through verified durable local Git-backed Framework 1.15.0 completion
- **Parent Outcome:** OUT-012
- **Authorized Scope:** local read/inspect/research; architectural design/spec; implementation planning; non-destructive in-scope Framework documentation/governance/template/test edits; pressure scenarios; validation/debug/corrective edits; local Git add/commit; Logical Checkpoints; Project Source/evidence/completion reconciliation
- **Explicitly Excluded:** push/publication/merge; destructive operations; Root/Project Location Binding mutation; external AI/provider disclosure; actual secret values; V2 TASK-044 mutation; unrelated Tasks; runtime/parser/interceptor/validator/CLI implementation
- **Validity:** TERMINATED at verified local TASK-045 completion; no future operation may rely on AUTH-012
- **Verification Requirement:** terminal — candidate `bc7f91c` passed AFFECTED `27/27` and exactly one RELEASE_FULL `33/33`; release evidence committed at `4278182`; terminal reconciliation must be freshly observed before external completion claim
- **Status:** TERMINATED
- **Expiry / Termination:** OUT-012 ACHIEVED / ACT-024 DONE / ENV-012 EXPIRED after verified Framework 1.15 local release acceptance
- **Evidence:** EVD-079, EVD-080; release evidence commit `4278182`

No DEL-* is created. `commit != push`; publication is not included.
## AUTH-013 — Framework 1.15 reconciliation persistent Goal authority
- **Authority Type:** USER_EXPLICIT_PERSISTENT_GOAL_AUTHORIZATION
- **Granted By:** ACTOR-001
- **Granted At:** 2026-09-10T10:58:43+07:00
- **Purpose / Outcome:** reconcile ProjectFramework to the Framework 1.15 line, cancel ProjectFramework 2.0/TASK-044, preserve TASK-044/TASK-046 provenance, correct current 1.15 drift if verified, and complete durable local verification
- **Parent Outcome:** OUT-013
- **Authorized Scope:** local read/inspect/research; design/spec and implementation planning; non-destructive in-scope Framework 1.15 documentation/governance/template/test corrections; Task/Project Source lifecycle reconciliation; historical artifact preservation; pressure scenarios/verifiers; validation/debug/corrective edits; local Git add/commit; Logical Checkpoints; final local evidence/reconciliation
- **Explicitly Excluded:** push/publication/PR/merge; destructive branch/worktree deletion or history rewrite; Root/Project Location Binding mutation; AI-ControlTower mutation; ProjectFramework 2.0 merge/cutover/runtime implementation; external AI/provider disclosure; actual secret values; unrelated Tasks; runtime/parser/interceptor/validator/CLI implementation
- **Validity:** TERMINATED at 2026-09-10T20:57:37+07:00; OUT-013 achieved and this authority permits no future execution
- **Verification Requirement:** written-spec review gate; current-surface classification; preserve historical artifacts; TASK-042/TASK-043 compatibility; affected verification; state-bound final candidate evidence; observed completion commit
- **Status:** TERMINATED
- **Terminal Lifecycle:** OUT-013 ACHIEVED / AUTH-013 TERMINATED / ACT-025 DONE / ENV-013 EXPIRED
- **Evidence:** EVD-082, EVD-083, EVD-084; frozen candidate `16664a8b61d1641a842210773c8f997178502be5`; one RELEASE_FULL 29/29 PASS; release evidence commit `009b4dbd5de93f56d5f9eb1c2ce1a1dedc4eedb7`

Historical metadata note: the superseded active `12-Authorization-Registry-r028-260910-1058.md` carried front-matter revision `27` and superseded `r026`. That pre-existing mismatch is archived unchanged; this revision is numbered and routed by the observed active filename lineage, not fabricated header provenance.

No DEL-* is created. `commit != push`; publication is not included.

## AUTH-014 — Framework 1.15 publication and Project upgrade preparation authority
- **Authority Type:** USER_EXPLICIT_PERSISTENT_GOAL_AUTHORIZATION
- **Granted By:** ACTOR-001
- **Granted At:** 2026-09-10T22:20:10+07:00
- **Purpose / Outcome:** publish the verified Framework 1.15 lineage to canonical `main`/`origin/main`, then prepare ProjectFramework's governed Direct-to-Latest `[Project Upgrade]` from local Framework pin 1.7.0 to target 1.15.0.
- **Parent Outcome:** OUT-014
- **Authorized Scope:** local read/inspect; fresh fetch/base/evidence verification; exact fast-forward integration to local `main`; non-force push of the exact verified Goal-checkpoint lineage to `origin/main`; fresh remote post-push verification; publication reconciliation; after publication, read-only `[Project Upgrade]` comparison, cumulative current->target assessment, Upgrade Preview, preservation/rollback/verification planning, and required Project Source evidence/checkpoint updates.
- **Explicitly Included Shared / External Effects:** exact non-force push to `captainhuke-dev/ProjectFramework` canonical `main` after fresh fast-forward/integration gate.
- **Not Yet Authorized:** mutation of active `FRAMEWORK-001`, Project Source Framework pin, root `PROJECT-BOOTSTRAP.md`, or other Brownfield upgrade surfaces. Those mutations require a materialized Upgrade Preview and separate explicit Human mutation approval after classification.
- **Explicitly Excluded:** force push; destructive history rewrite; branch/worktree deletion; unrelated Root/Binding mutation; AI-ControlTower mutation; ProjectFramework 2.0/V2 cutover/runtime; external disclosure; actual secret values; unrelated Tasks; runtime/parser/validator/CLI implementation.
- **Verification Requirement:** Base Freshness + evidence-validity gate; exact fast-forward topology; clean/explained worktrees; remote OID confirmation after push; target Framework 1.15 identity/tree confirmation before upgrade Preview; preservation/rollback classification before any upgrade mutation.
- **Status:** ACTIVE
- **Evidence:** EVD-085; EVD-084 and OUT-013 release evidence as prior verified release basis.

`commit != push`; only the exact canonical-main publication described above is currently authorized. `[Project Upgrade]` comparison/Preview does not itself authorize mutation.