...
```yaml
project_uuid: "00575e76-17ce-4dd3-ad24-377494a4a45b"   # immutable
project_id: "PROJECTFRAMEWORK"       # stable human-readable ID
project_name: "ProjectFramework"   # mutable display name
```
...
project_location_binding:
  ...
  # No generic non-Drive external File Storage is applicable for this Project.
```
...
Framework Distribution Root (upstream current 1.8.0): Framework-Source/
Google Drive: NOT_APPLICABLE
External File Storage: NOT_APPLICABLE
```

The current reusable Framework distribution directory is `Framework-Source/` and remains distinct from this Project's authoritative `Project-Source/`. TASK-038 migrated the upstream distribution root to Framework `1.8.0`; this repository-structural migration does not auto-upgrade ProjectFramework's active local `FRAMEWORK-001`, which remains pinned to Framework `1.7.0` / Schema `1.0.0` until separately governed `[Project Upgrade]` work changes that pin.
