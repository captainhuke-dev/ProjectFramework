...
supersedes: "91-Project-Management-Control-r061-260913-1457.md"
...
- **Success Criteria:** approved written design/spec; implementation plan after user spec review; one-session FAST_PATH + bounded compatible ASSESSED_PATH contract; MAJOR exclusion; exact release-evidence reuse with invalidation rules; Project-specific affected verification; interruption recovery; canonical self-host chaining/stops; scenarios 505-528; independent review of TASK-052 implementation; AFFECTED + one final 1.18 RELEASE_FULL; terminal local evidence/commit; no unauthorized publication/Root/Binding/runtime effects.
- **Current Evaluation:** Design Sections 1-3 USER_APPROVED; written spec SELF_REVIEWED 15/15; implementation planning blocked only by the required written-spec user review gate.
- **Related AUTH / ACT / ENV:** `AUTH-020 ACTIVE` / `ACT-032 IN_PROGRESS` / `ENV-020 ACTIVE`.
...
- **Remaining Work:** user written-spec review -> implementation plan -> TDD/implementation -> independent review -> AFFECTED -> frozen candidate RELEASE_FULL -> terminal local evidence.
