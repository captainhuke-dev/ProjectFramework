...
## Framework 1.21.0 V3 Forward-Port Runtime Control Contract Semantics

Framework `1.21.0` preserves Schema `1.0.0` and all prior governance (including the Framework 1.20 Compositional State Binding Hub, which remains authoritative) while adding the Durable Runtime Control Contract: `RUNTIME_CONTRACT`, `RUNTIME_EVENT`, `EXECUTION_CHECKPOINT`, `EFFECT_POLICY`, `EFFECT_PERMIT`, and `RLM_EXECUTION_PROFILE` record semantics; Supervisor/control generation (`Supervisor decision ≠ AUTH`; `Heartbeat ≠ completion evidence`; fence identity `(runtime_generation, fence_epoch)`); runtime liveness projection subordinate to the Execution Ownership Grant; Effect Gateway mediation with single-use bound Effect Permits (`Effect Permit ≠ AUTH-*`); source-native ambiguity reconciliation with no blind retry; bounded continuation (`WAIT | BLOCKED | MANUAL_RESOLUTION_REQUIRED | BUDGET_EXHAUSTED | WAITING_FOR_REAUTHORIZATION`); provider-neutral RLM profile (`max_recursion_depth`, hierarchical budgets, no child authority/budget minting, no Gateway bypass); `Cancellation ≠ rollback`; `Compensation ≠ history erasure`; runtime/event-schema version pinning (`BACKWARD_COMPATIBLE | REQUIRES_MIGRATION | INCOMPATIBLE`); and the AI-ControlTower Python 3.14 Supervisor conformance handoff (separate implementation Task; Prime Agent optional mapping). Optional `Project-Execution/` TASK-059 starters (`runtime-contract.md`, `runtime-event-journal.md`, `execution-checkpoint.md`, `effect-policy.md`, `rlm-executor-profile.md`) are applicability-driven. No new semantic slot, Stable-ID family, Registered Command, or runtime is introduced.
...
`R4_CTX ≠ Risk R4` (Risk remains `R0–R3`); `Contract ≠ Authority`; `Verification PASS ≠ Task DONE`; Execution State never mutates canonical Task lifecycle; `VERIFIED ≠ INTEGRATION_ELIGIBLE ≠ MERGED`; local-only `DONE` ≠ publication; Brownfield Tasks are not retrofitted. Optional `Project-Execution/` contract starters (`plan-contract.md`, `task-contract.md`, `task-record.md`, `verification-record.md`, `executor-profile.md`, `project-adapter.md`, `integration-reconciliation.md`) are applicability-driven. No new semantic slot, Stable-ID family, or runtime is introduced.
