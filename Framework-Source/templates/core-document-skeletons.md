...
Framework `1.9.1` hardens response finalization: resolve Project Bootstrap before the first Project-governed response in each chat when available; read-only/status/diagnostic/failure-report responses are not exempt; every Project-governed final response, including exceptional/early-return paths, must still run the Response Close Completeness Gate. Material work keeps all independent mutation gates.
