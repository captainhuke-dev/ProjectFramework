...
Framework `1.9.1` current vendor-facing Project Settings / Project Instructions use exactly two bootstrap locators plus one rule:
...
Current Framework `1.9.1` vendor settings do not require the legacy five labels. When legacy/Brownfield settings or internal bootstrap representations contain angle-bracket placeholders, those values remain **unset / not configured** and never authorize fallback.
