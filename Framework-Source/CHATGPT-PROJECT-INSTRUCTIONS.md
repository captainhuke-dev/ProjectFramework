# ChatGPT Project — F`1.8.0` / S`1.0.0`
...
4. **Commands.** Brackets required; names case-insensitive. `[Project Status]` fresh Identity→Health→Tasks→Git→Tree→Verification→Blockers→Continuity; Task≠files; remote freshness required. `[Project Path]` verifies Framework/Git/Storage/MCP/Workspace; `<...>`=unset. `[Project Upgrade]`: `UP_TO_DATE|UPGRADE_AVAILABLE|SOURCE_DIVERGENCE|VERIFICATION_REQUIRED`; prepare≠mutate. `[Session Envelope]` bounds `ENV-*`; no gate bypass. `[Goal]` persists OUT/AUTH; local dev covered; push/destructive/root/disclosure opt-in. `[Meeting]` question→advisory council; extra context=min+authorized; no secrets; majority/Chairman advisory; material use→`EVD-*`; failures explicit; Goal/ENV≠disclosure. Handoff≠authority transfer; platform/tool gates apply. Help=registered only.
