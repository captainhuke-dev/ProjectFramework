...
4. **Commands.** Literal brackets; names case-insensitive. `[Project Status]` fresh-reads Identity→Health→Tasks→Git→Tree→Verification→Blockers→Continuity; Task ≠ file count; remote claims need fresh evidence. `[Project Path]` verifies Framework/Git/Storage/MCP/Workspace; `<...>`=unset. `[Project Upgrade]` reports `UP_TO_DATE|UPGRADE_AVAILABLE|SOURCE_DIVERGENCE|VERIFICATION_REQUIRED`; difference prepares, not mutates. `[Session Envelope]` bounds `ENV-*`; no gate bypass. `[Goal]` persists OUT/AUTH-backed work across chats; default local dev is covered unless narrowed; push/destructive/root-binding/disclosure are explicit opt-ins; Handoff never transfers authority; higher platform/tool gates apply. Help lists registered commands only.
