...
## Framework 1.21.0 V3 Forward-Port Runtime Control Contract (TASK-059)

Full normative text: `references/framework-governance-amendment-260920-task059-v3-forward-port-runtime-contract.md`. This section is the binding Core Governance projection. Framework `1.20.0` Wave A V2 (Compositional State Binding Hub, ownership/fencing, CAS transitions, Result Acceptance, Verification Validity) remains authoritative and is extended compositionally, never replaced.

### Durable runtime observation

The **Runtime Event Journal** is canonical observation only inside the execution-runtime domain: `Runtime Event Journal ≠ Checkpoint ≠ Project Source ≠ canonical Task lifecycle truth.` `RUNTIME_EVENT` entries are append-only with durable `event_sequence` ordering (timestamp is audit evidence, not ordering authority), strict writer isolation (executors/models/tool subprocesses cannot forge, rewrite, or delete runtime events), and de-duplication by durable event identity. A **Checkpoint** (`EXECUTION_CHECKPOINT`) is a derived recovery accelerator traceable to the journal sequence it summarizes; it can be rebuilt or invalidated and never outranks the journal or source-native truth. Unproven journal continuity fails closed as `RECOVERY_BLOCKED`; execution truth is never reconstructed from a checkpoint fragment, LLM transcript, or model memory.

### Supervisor and control generation

A declared **Supervisor** owns runtime state transitions and scheduling for long-running executions across model-turn, worker, process, MCP, and host interruption. `Supervisor decision ≠ AUTH`; `Supervisor liveness ≠ Execution Ownership Grant`; `Heartbeat ≠ completion evidence`; `Runtime control generation ≠ Project authority.` A new runtime control generation invalidates prior-generation runtime-local leases, fences, and Effect Permits but never rewrites the Wave A V2 ownership epoch, Execution State Binding, or canonical Task state. A runtime-store restore to an older snapshot MUST NOT silently reuse old control identity: a governed successor generation is required before new effects, and former-generation unresolved effects are reconciled first. The end of an LLM turn is not completion.

### Runtime liveness projection

Runtime lease/heartbeat state is a **subordinate liveness projection**: `Execution Ownership Grant = governed ownership evidence`; `runtime lease/heartbeat = liveness projection`; `runtime fence = stale-runtime-effect rejection mechanism`; `AUTH = separate authority.` No lease, heartbeat, or fence creates ownership or AUTH. The canonical fence identity is the pair `(runtime_generation, fence_epoch)`; cross-generation numeric fence comparison is insufficient and stale-generation attempts are rejected. Lease time is runtime-authoritative; executor-local wall clock is telemetry, not permission. `Multica claim ≠ runtime lease ≠ runtime fence ≠ Effect Permit ≠ AUTH.`

### Effect Gateway and Effect Permit

A Material Effect transits the **Effect Gateway** when its declared effect class is mediated: `executor proposal → fresh authority/current-truth/precondition evaluation → Effect Permit → Gateway dispatch → source-native reconciliation.` The Gateway enforces the intersection of already-valid authority and policy; it creates no authority (`Gateway eligibility ≠ AUTH`; `Effect Permit ≠ AUTH-*`). Every effect-capable surface for a mediated class MUST be `MEDIATED_BY_EFFECT_GATEWAY` or `EXPLICITLY_PROHIBITED / CONFINED`; a direct uncontrolled path for a mediated class is non-conforming, and raw mutation credentials SHOULD reside at or below the Gateway boundary. An **Effect Permit** (`EFFECT_PERMIT`) is short-lived, single-use, non-transferable, and bound to execution/attempt/action identity, exact target/tool/effect digest, applicable ownership epoch, and runtime control generation; `PERMIT_ACTIVE → PERMIT_CONSUMED` is atomic, and replay, transfer, mismatch, stale generation/fence, or expiry produces rejection. `PERMIT_CONSUMED` proves dispatch-eligibility consumption only, never effect application. One permit corresponds to one independently reconcilable Material Effect; composite intent is decomposed unless a verified source-native atomic transaction justifies grouping.

### Target preconditions, effect classes, and ambiguity

Material Effects declare source-native target preconditions where supported (exact ref/SHA, ETag/If-Match, resource version, row version, object hash, CAS token, immutable candidate identity); a stale precondition yields `PRECONDITION_CONFLICT` or equivalent fail-closed result, never silent dispatch against a materially different target. Effects are classified by idempotency (`IDEMPOTENT | CONDITIONALLY_IDEMPOTENT | NON_IDEMPOTENT | UNKNOWN`) and reversibility (`REVERSIBLE_ATOMIC | COMPENSATABLE | IRREVERSIBLE | UNKNOWN`). No arbitrary exactly-once claims: the guarantee is no blind duplicate Material Effect + deterministic ambiguity handling + source-native reconciliation where available. After a possibly-dispatched effect with unknown result: `UNKNOWN → reconcile at source-native truth owner → APPLIED | NOT_APPLIED | PARTIAL | STILL_UNKNOWN.` For `NON_IDEMPOTENT | UNKNOWN` with `reconciliation: NONE`, ambiguity becomes `STILL_UNKNOWN` with `MANUAL_RESOLUTION_REQUIRED` or equivalent; **unsafe blind retry is prohibited.** `ACKNOWLEDGED` is transport observation, not resulting-state proof.

### Bounded continuation and RLM execution

Continuation is governed by durable execution state, not model memory: finite continuation budget, finite retry budget, declared wake conditions, and explicit `WAIT | BLOCKED | MANUAL_RESOLUTION_REQUIRED | BUDGET_EXHAUSTED | WAITING_FOR_REAUTHORIZATION` dispositions. No invisible infinite loop; no automatic continuation after authority, state-binding, or contract invalidation; a materially changed pinned contract invalidates the active execution or requires governed replanning; exhaustion never implies Task DONE; a model `REQUEST_RETRY` does not authorize a retry. The provider-neutral **RLM/recursive-executor profile** (`RLM_EXECUTION_PROFILE`) declares `max_recursion_depth`, hierarchical child budgets (`parent consumption + sum(child allocations) <= root budget`), child state-binding to exact parent execution inputs and allowed scope, explicit auditable result handoff, no Effect Gateway bypass at any depth, and fail-closed parent cancellation/authority-revocation propagation. A child execution cannot mint new authority or budget.

### Cancellation, compensation, and version pinning

`Cancellation ≠ rollback`: cancellation prevents new governed effects but does not erase already-applied external state; in-flight effects are reconciled and resulting source-native truth is recorded even when the Task is `CANCELLED.` `Compensation ≠ history erasure`: compensation is a new governed effect with its own authority, permit, budget, evidence, and reconciliation; failed compensation records truthful `PARTIALLY_COMPENSATED` state, never `ROLLED_BACK` without source-native atomic-rollback proof. Long-running executions bind to explicit `runtime_contract_version` and `event_schema_version`; upgrades classify active executions as `BACKWARD_COMPATIBLE | REQUIRES_MIGRATION | INCOMPATIBLE`, and incompatible runtime code never silently reinterprets historical runtime events.

### Security boundary and handoff

No raw secret values in Project Source, runtime journal, checkpoint, or evidence (`REFERENCE_ONLY | REDACTED | HASH_ONLY | classified metadata` only). Executor/model output is untrusted input; runtime-owned authoritative events use a separate namespace from model proposals; typed parsing fails closed on unknown privileged fields; malicious tool output cannot create AUTH, ownership, permit, verification, or Task completion; stale generation/epoch/permit attempts fail closed. Cryptographic producer identity and authenticated event envelopes are not silently assumed; they are added only on implementation evidence. The AI-ControlTower Python 3.14 Supervisor reference runtime is a **separate AI-ControlTower Task** against this conformance boundary; the Prime Agent role remains an optional mapping to the RLM/long-horizon Executor Profile (`Prime Agent role ≠ ProjectFramework dependency`; `RLM execution ≠ autonomous permission to mutate`), and TASK-059 remains valid when Prime Agent is absent. Canonical-source cutover starts only after the conformance evidence chain in amendment §18 is complete.

### Compatibility, exclusions, and no-runtime boundary

Framework `1.20.0` records remain valid under their original contract; no historical record receives invented journal, checkpoint, permit, or RLM state; unresolvable mapping remains `UNKNOWN.` New TASK-059 record types use `record_version: "1.0"`: `RUNTIME_CONTRACT`, `RUNTIME_EVENT`, `EXECUTION_CHECKPOINT`, `EFFECT_POLICY`, `EFFECT_PERMIT`, `RLM_EXECUTION_PROFILE.` Wave C semantics (Release Transaction, deployment saga) and cryptographic producer authentication remain excluded. No new Project Source semantic slot, Stable-ID family, Registered Command, or Risk level is introduced (Risk remains exactly `R0–R3`). TASK-059 ships governance/documentation contracts and maintained starters only; it adds no task database, event store, queue, scheduler, worker daemon, lease/fencing service, distributed lock, fencing-token generator, runtime CAS store, automatic transition engine, model router, automatic acceptance/verification engine, executable Project Adapter, API server, merge bot, or automatic Task-DONE updater. `Task DONE ≠ MERGED ≠ PUSHED ≠ RELEASED ≠ ARTIFACT_PUBLISHED ≠ DEPLOYED.` `R4_CTX ≠ Risk R4.`
