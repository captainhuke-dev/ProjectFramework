...
- **parallelizable_with:** `[]`
- **priority:** `HIGH`
- **readiness:** `USER_APPROVED_DIRECTION / WRITTEN_SPEC_SELF_REVIEWED / WRITTEN_SPEC_USER_APPROVED / HERMES_DESIGNATED_EXECUTOR / HERMES_RUNTIME_BINDING_UNVERIFIED / FAIL_CLOSED / IMPLEMENTATION_NOT_STARTED`
...
- **Execution Handoff Evaluation:** `FAIL_CLOSED / HERMES_RUNTIME_BINDING_UNVERIFIED`. No Hermes tool/runtime endpoint is exposed in the current CEO execution environment, and ProjectFramework has no active project-local runtime/tool binding artifact proving Hermes against this exact workspace/task. Selection is durable; execution remains blocked until fresh binding + Task Ready Gate PASS.
- **Exact Next Step:** fresh-resolve Hermes runtime/tool/workspace binding for TASK-059 and run the Task Ready Gate; if PASS, hand Phase 0/RED-first execution to Hermes; otherwise remain fail-closed without Executor substitution.
