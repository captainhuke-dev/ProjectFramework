...
- **parallelizable_with:** `[]`
- **priority:** `HIGH`
- **readiness:** `USER_APPROVED_DIRECTION / WRITTEN_SPEC_SELF_REVIEWED / WRITTEN_SPEC_USER_APPROVED / HERMES_DESIGNATED_EXECUTOR / TASK_READY_GATE_PASS / EXECUTED / RELEASE_FULL_PASS_RUN_1 / PUSHED`
...
- **Exact Next Step:** ACTOR-001 decision on PR vs direct merge of the pushed branch (frozen candidate `0d51582b553953a176dcb3f40fe942e177588028`), then tag `v1.21.0` + GitHub Release per `tag_convention: "v{framework_version}"`. Separately governed follow-ups: (1) governed post-merge self-host reconciliation to `1.21.0 / 1.0.0`; (2) separate AI-ControlTower Python Supervisor implementation Task against amendment §18; (3) separate Canonical Source Cutover Task after accepted runtime handoff; (4) standalone feature-development freeze only after canonical readback.
