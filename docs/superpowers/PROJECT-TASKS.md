...
- **Verification Progress:** TDD RED `TASK048_RED 9/18` expected; structural GREEN `TASK048_STRUCTURAL 18/18 PASS`; cumulative AFFECTED `TASK048_AFFECTED 47/47 PASS`; after release-verifier UTF-8 repair, AFFECTED rerun `47/47 PASS`; pressure scenarios `1–468` contiguous/unique.
...
- **Candidate State:** `REFREEZE_READY / RELEASE_FULL_NOT_RUN`; the next Task 6 freeze commit after this invalidation record becomes the replacement final Framework 1.16 candidate and its exact identities are bound by post-commit readback and Task 7 evidence.
